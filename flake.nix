{
  ...
  outputs = { self, nixpkgs, flake-utils, solidity, ethereum-tests, nix-bundle-exe, ... }:
    flake-utils.lib.eachDefaultSystem (system:
      let
        ...
        hevmUnwrapped = (with pkgs.pkgsStatic; lib.pipe (
        ...
        then hevmWrapped
          ...
          mkdir -p $out/bin
          cp ${hevmUnwrapped}/bin/hevm $out/bin/
      ...
      in rec {
        ...
        packages.redistributable = pkgs.haskell.lib.dontCheck hevmRedistributable;
        packages.default = packages.hevm;

        # --- apps ----

        apps.hevm = flake-utils.lib.mkApp { drv = (pkgs.haskell.lib.dontCheck hevmWrapped); };
